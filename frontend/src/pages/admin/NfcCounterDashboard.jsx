...
const NfcCounterDashboard = ({ user }) => {
  const { t } = useLanguage();
  ...
  const filteredCounters = counters.filter(c => {
    ...
    return uidClean.includes(term) || 
           uidFormatted.includes(term) || 
           title.includes(term) || 
  ...
  if (!isAdmin) {
    return (
      <div className="admin-access-denied-container">
        <div className="royal-card denied-card" style={{ maxWidth: '500px', margin: '60px auto', textAlign: 'center' }}>
          <div className="denied-icon-wrapper" style={{ marginBottom: '24px' }}>
  ...
  return (
    <div className="nfc-dashboard-container animate-fade-in" style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px 24px 60px 24px' }}>
      ...
      <header className="nfc-dashboard-header" style={{ marginBottom: '40px', textAlign: 'center' }}>
      ...
      <div className="nfc-control-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px', marginBottom: '32px' }}>
        ...
        <div className="royal-card control-panel-card" style={{ padding: '20px 24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div className="search-input-wrapper" style={{ position: 'relative' }}>
              <input 
                type="text" 
                placeholder="Search by UID, Title, Author, or ISBN..." 
                value={searchQuery}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <label style={{ fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--accent, #d4af37)', fontWeight: 'bold' }}>
                Filter Min Tap Counter: {minCounterFilter}
              </label>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <input 
                  ...
                  min="0" 
                  max="100" 
                  value={minCounterFilter}
                  onChange={handleMinCounterChange}
                  style={{ flex: 1, accentColor: 'var(--accent, #d4af37)', cursor: 'pointer' }}
                />
                <input 
                  type="number" 
                  min="0"
                  ...
                  onChange={handleMinCounterChange}
                  className="royal-input"
                  style={{ width: '70px', padding: '6px 8px', fontSize: '0.85rem', textAlign: 'center' }}
                />
              </div>
            </div>
      ...
      {/* Notifications area */}
      {successMsg && (
        <div className="success-banner animate-fade-in" style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'rgba(34, 139, 34, 0.15)', border: '1px solid rgba(34, 139, 34, 0.4)', borderRadius: '8px', padding: '14px 20px', color: '#7cfc00', marginBottom: '24px', fontSize: '0.9rem' }}>
          <CheckCircle size={18} /> {successMsg}
        </div>
      )}

      {errorMsg && (
        <div className="error-banner animate-fade-in" style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'rgba(178, 34, 34, 0.15)', border: '1px solid rgba(178, 34, 34, 0.4)', borderRadius: '8px', padding: '14px 20px', color: '#ff6b6b', marginBottom: '24px', fontSize: '0.9rem' }}>
          <AlertTriangle size={18} /> {errorMsg}
        </div>
      )}

      {/* Diagnostic Ledger Table Card */}
      <div className="royal-card ledger-table-card" style={{ overflow: 'hidden', padding: 0 }}>
        {loading ? (
          ...
          <div style={{ overflowX: 'auto' }}>
            <table className="diagnostic-table" style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.85rem' }}>
              <thead>
                <tr style={{ background: 'rgba(212, 175, 55, 0.05)', borderBottom: '1px solid rgba(212, 175, 55, 0.15)' }}>
