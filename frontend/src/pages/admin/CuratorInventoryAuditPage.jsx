...
import { ClipboardCheck, Smartphone, RefreshCw, Check, AlertTriangle, Sparkles, Play, StopCircle, Search, Compass, BookOpen, ArrowLeft, CheckCircle2, XCircle } from 'lucide-react';
import { Link } from 'react-router-dom';
import { useLanguage } from '../../i18n/LanguageContext';
import { fetchBooks } from '../../services/libraryApi';
import { startAudit, scanItem, completeAudit, getActiveAudit } from '../../services/inventoryAuditApi';
import './CuratorInventoryAuditPage.css';
const CuratorInventoryAuditPage = ({
  user
}) => {
  const {
    t
  ...
  if (loading) {
    return <div className="audit-container" style={{
      display: 'flex',
      ...
      alignItems: 'center'
        ...
        textAlign: 'center'
          ...
          <RefreshCw className="spin-icon" size={40} style={{
          color: '#d4af37',
          marginBottom: '12px'
        }} />
  ...
  return <div className="audit-container">
      ...
      {/* Message Banners */}
      {errorMessage && <div className="royal-card" style={{
      ...
      {successMessage && <div className="royal-card" style={{
      ...
      {!activeAudit ? (/* Empty/Inactive State */
    <div className="audit-empty-state">
          ...
          <button type="button" onClick={handleStartSession} disabled={actionLoading} className="royal-btn lookup-btn" style={{
        display: 'flex',
        alignItems: 'center',
        gap: '10px',
        fontSize: '1rem',
    ...
    <div className="audit-dashboard-grid">
        ...
        gap: '24px'
            ...
            {/* Scan Area Card */}
            {activeAudit.status === 'ACTIVE' && <div className="audit-panel scanner-card">
                <h3 style={{
            color: '#d4af37',
            ...
            lineHeight: '1.5'
                ...
                <form onSubmit={handleScanItemSubmit} className="scanner-flex-row">
                  <input type="text" className="royal-input scanner-input" value={scanInput} onChange={e => setScanInput(e.target.value)} placeholder={t("str_5286", "Enter Book ISBN or physical barcode...")} disabled={actionLoading} />
                  <button type="submit" disabled={actionLoading || !scanInput.trim()} className="royal-btn" style={{
              padding: '0 24px'
            }}>
                    <span>{t('auto_3359', 'Validate Code')}</span>
                  </button>
                  <button type="button" onClick={handleNfcRead} disabled={actionLoading || isNfcReading} className={`royal-btn-secondary ${isNfcReading ? 'loading-btn' : ''}`} style={{
              display: 'flex',
              alignItems: 'center',
              gap: '8px',
              padding: '0 20px',
            ...
            textAlign: 'center'
                    ...
                    <Smartphone size={24} className="gold-glow-icon animate-pulse" style={{
              color: '#d4af37',
              marginBottom: '6px'
            }} />
                    <p style={{
              ...
              opacity: 0.8,
              margin: 0
            }}>{t('auto_3360', 'Tap catalog tag to browser device NFC antenna...')}</p>
                  </div>}
              </div>}
            ...
            <div className="audit-panel">
              ...
              fontSize: '0.85rem',
              opacity: 0.7
            }}>{books.length} {t("str_5287", "Catalogue entries")}</span>
              </div>

              {/* Premium Flag-based Filter Row */}
              <div className="audit-flag-filters" style={{
            display: 'flex',
            gap: '8px',
            overflowX: 'auto',
            padding: '12px 16px',
            ...
            marginBottom: '14px'
              ...
              key: 'ISBN_MISMATCH',
              label: 'Mismatched ISBNs'
            }, {
              key: 'BAD_DESCRIPTION',
              label: 'Bad Descriptions'
            }, {
              key: 'BAD_GENRE',
              label: 'Bad Genres'
            }, {
              ...
              key: 'HAS_NOTES',
              label: 'Has Notes'
            }].map(filter => <button key={filter.key} type="button" onClick={() => setSelectedFilterFlag(filter.key)} style={{
              padding: '6px 12px',
              borderRadius: '16px',
              fontSize: '0.75rem',
              fontWeight: '500',
              ...
              return <div key={book.isbn} style={{
                display: 'flex',
                flexDirection: 'column',
                gap: '8px',
                ...
                paddingBottom: '12px'
                      ...
                      <div className={`catalog-item-card ${isFullyAudited ? 'scanned' : isPartiallyAudited ? 'scanned' : 'missing'}`} onClick={() => setExpandedBookIsbn(expandedBookIsbn === book.isbn ? null : book.isbn)} style={{
                  cursor: 'pointer',
                  transition: 'all 0.2s',
                  background: expandedBookIsbn === book.isbn ? 'rgba(212, 175, 55, 0.04)' : 'rgba(255,255,255,0.01)'
                    ...
                    justifyContent: 'center'
                      ...
                      gap: '8px'
                            ...
                            {(auditNotes[book.isbn] || '').trim() && <span style={{
                        fontSize: '0.65rem',
                      ...
                      }}>{t('auto_3362', 'Note')}</span>}
                          </h4>
                          <p style={{
                      marginBottom: '2px'
                    }}>{Array.isArray(book.authors) ? book.authors.join(', ') : book.author}</p>
                          <p style={{
                      fontSize: '0.74rem',
                      opacity: 0.55
                    }}>{t("str_5288", "ISBN:")} {book.isbn}</p>
                        </div>
                    ...
                    alignItems: 'flex-end'
                      ...
                      fontSize: '0.8rem',
                      fontWeight: '500'
                    }}> {t("str_5289", "Scanned:")} {scannedCountForThis} {t("str_5290", "/ Expected:")} {book.totalCopies || 0}
                          </span>
                          {isFullyAudited ? <span className="status-badge scanned-badge">{t('auto_3363', 'Verified')}</span> : isPartiallyAudited ? <span className="status-badge scanned-badge" style={{
                      color: '#fbbf24',
                      borderColor: '#fbbf24'
                    }}>{t('auto_3364', 'Partial')}</span> : <span className="status-badge missing-badge">{t('auto_3365', 'Missing')}</span>}
                        </div>
                      </div>
                  ...
                  gap: '12px'
                    ...
                    fontWeight: '600',
                    fontFamily: '"Outfit", sans-serif'
                  }}>{t('auto_3366', 'Copy Checklist Verification')}</span>
                    ...
                    gap: '10px'
                      ...
                      key: 'descriptionCorrect',
                      label: 'Description correct'
                    }, {
                      key: 'genreCorrect',
                      label: 'Genre correct'
                    }, {
                      ...
                      return <label key={item.key} style={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: '8px',
                        cursor: 'pointer',
                        ...
                        color: 'rgba(255, 255, 255, 0.85)'
                                  ...
                                  <input type="checkbox" checked={currentVal} onChange={e => updateChecklist(book.isbn, item.key, e.target.checked)} style={{
                    ...
                    gap: '6px'
                      ...
                      color: '#d4af37',
                      fontWeight: '500'
                    }}>{t("str_5291", "Curator Audit Notes:")}</span>
                            <textarea className="royal-input" placeholder={t("str_5292", "Capture custom free-text notes for this book copy...")} value={auditNotes[book.isbn] || ''} onChange={e => updateNote(book.isbn, e.target.value)} style={{
                      width: '100%',
                      minHeight: '60px',
                      fontSize: '0.8rem',
                      padding: '8px 12px',
        ...
        gap: '24px'
          ...
          border: '1px solid rgba(212, 175, 55, 0.15)'
            ...
            marginTop: '10px'
              ...
              paddingBottom: '8px'
                ...
                opacity: 0.8,
                fontSize: '0.88rem'
              }}>{t("str_5293", "Session status:")}</span>
                  <span style={{
                fontWeight: '600',
                color: activeAudit.status === 'ACTIVE' ? '#fbbf24' : '#34d399',
              ...
              paddingBottom: '8px'
                ...
                opacity: 0.8,
                fontSize: '0.88rem'
              }}>{t("str_5294", "Total expected copies:")}</span>
                  <span style={{
                fontWeight: '600'
              }}>{totalExpected}</span>
                </div>
              ...
              paddingBottom: '8px'
                ...
                opacity: 0.8,
                fontSize: '0.88rem'
              }}>{t("str_5295", "Total copies found:")}</span>
                  <span style={{
                fontWeight: '600',
                color: '#34d399'
              }}>{scannedCount}</span>
              ...
              paddingBottom: '4px'
                ...
                opacity: 0.8,
                fontSize: '0.88rem'
              }}>{t("str_5296", "Total missing copies:")}</span>
                  <span style={{
                fontWeight: '600',
                color: '#ff7b72'
              }}>{Math.max(0, totalExpected - scannedCount)}</span>
                </div>
              </div>

              {activeAudit.status === 'ACTIVE' ? <button type="button" onClick={handleCompleteSession} disabled={actionLoading} className="royal-btn" style={{
            width: '100%',
            marginTop: '24px',
            display: 'flex',
            alignItems: 'center',
            ...
            textAlign: 'center'
              ...
              gap: '6px'
                  ...
                  <button type="button" onClick={handleStartSession} className="royal-btn-secondary" style={{
              width: '100%',
              marginTop: '12px'
            }}>
          ...
          border: '1px solid rgba(255,255,255,0.05)'
            ...
            fontWeight: '600',
            margin: '0 0 10px 0'
          }}>{t("str_5297", "Shelf audit instructions:")}</h4>
              <ul style={{
            fontSize: '0.78rem',
            opacity: 0.75,
            paddingLeft: '16px',
