...
export default function CuratorCheckoutsPage({
  user
}) {
  const {
    t
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [checkoutsRes, booksData, usersRes, settingsRes] = await Promise.all([fetchCheckouts(), fetchBooks(), getAllUsers(), getCheckoutSettings()]);
  ...
  const handleBulkApproveReturns = async () => {
    const adminId = user?.uid || user?.id || '';
    if (!adminId) return;
    const targets = checkouts.filter(c => c.status === 'REQUESTED_RETURN').filter(c => c.locationVerified === true);
  ...
  const renderBookCell = (bookId, copyNo = null) => {
    ...
    if (!book) {
      return <div className="ledger-book-cell">
          <BookOpen className="cell-icon text-muted" size={16} />
          <div>
            <div className="cell-primary-title">{t('admin.unknownVolume', 'Unknown volume')}</div>
            <div className="cell-sub-detail text-muted">{t("str_5182", "ISBN:")} {bookId}</div>
            {copyNo && <span className="gold-copy-badge" style={{
            display: 'inline-flex',
            alignItems: 'center',
            gap: '4px',
            fontSize: '0.65rem',
    ...
    return <div className="ledger-book-cell">
        ...
        <div>
          ...
          <div className="cell-sub-detail text-accent" style={{
          display: 'flex',
          alignItems: 'center',
          gap: '8px',
          flexWrap: 'wrap'
        }}>
            <span>{t("str_5184", "ISBN:")} {bookId}</span>
            {copyNo && <span className="gold-copy-badge" style={{
            display: 'inline-flex',
            alignItems: 'center',
            gap: '4px',
  ...
  const isContactIncomplete = memberId => {
    ...
    const addressParts = [member.houseNo, member.street, member.city, member.pinCode].filter(Boolean).map(v => String(v).trim()).filter(v => v !== 'null' && v !== '');
  ...
  const renderMemberCell = (memberId, tx) => {
    ...
    const addressParts = member ? [member.houseNo, member.street, member.city, member.pinCode] : [];
    const addressVal = addressParts.filter(Boolean).map(v => String(v).trim()).filter(v => v !== 'null' && v !== '').join(', ');
    return <div className="ledger-member-cell">
        ...
        <div>
          ...
          <div className="cell-sub-detail text-muted font-mono" style={{
          fontSize: '0.65rem'
        }}>{t("str_5186", "UID:")} {memberId ? `${memberId.slice(0, 8)}...` : '—'}</div>
          
          {showContactDetails && <div className="member-contact-details animate-slide-down">
              {phoneVal ? <div className="contact-detail-item">
                  <Phone size={10} className="contact-icon text-accent" />
                  <span className="contact-text font-mono">{phoneVal}</span>
                </div> : <div className="contact-detail-item empty-detail">
              ...
              {addressVal ? <div className="contact-detail-item">
  ...
  const renderStatusBadge = (status, ntagUid) => {
    switch (status) {
      case 'REQUESTED_CHECKOUT':
        return <span className="ledger-status-badge badge-checkout-req animate-pulse">
            <Clock size={12} /> {t('admin.checkoutReqBadge', 'Checkout Req')}
          </span>;
      case 'REQUESTED_RETURN':
        return <span className="ledger-status-badge badge-return-req animate-pulse">
            <Clock size={12} /> {t('admin.returnReqBadge', 'Return Req')}
          </span>;
      case 'CHECKED_OUT':
        return <span className="ledger-status-badge badge-checkedout">
      ...
      case 'RETURNED':
        return <span className="ledger-status-badge badge-returned">
            <CheckCircle size={12} /> {t('admin.returnedBadge', 'Returned')}
          </span>;
      case 'REJECTED':
        return <span className="ledger-status-badge badge-rejected">
  ...
  return <div className="curator-checkouts-container animate-fade-in">
      {/* Page Header */}
      ...
      <section className="circulation-analytics-grid">
        ...
        <div className="royal-card analytic-summary-card" style={{
        display: 'flex',
        flexDirection: 'column',
        gap: '8px',
        justifyContent: 'center'
      }}>
          <button onClick={loadData} className="ledger-refresh-btn" disabled={loading} style={{
          width: '100%',
          padding: '6px 12px'
        }}>
          ...
          <button onClick={() => setShowSandbox(prev => !prev)} className="royal-btn" style={{
          width: '100%',
          fontSize: '0.8rem',
          padding: '6px 12px',
      ...
      {showSandbox && <section className="royal-card curator-sandbox-card animate-fade-in" style={{
      ...
      boxShadow: "0 8px 32px var(--card-shadow)"
        ...
        marginBottom: '20px'
          ...
          borderRadius: '12px',
          fontWeight: 'bold'
        }}>{t('auto_3260', 'Active Sandbox')}</span>
          </div>
        ...
        gap: '24px'
          ...
          gap: '16px'
            ...
            paddingBottom: '6px',
            margin: 0
          }}>{t('auto_3261', 'Simulate Patron Return')}</h3>
            ...
            gap: '6px'
              ...
              fontSize: '0.9rem'
                  ...
                  <option value="" style={{
                background: '#121212'
              }}>{t('auto_3263', '-- Select a Book --')}</option>
                  {books.map(b => <option key={b.isbn} value={b.isbn} style={{
                background: '#121212'
              }}>{b.title} ({b.isbn})</option>)}
                </select>
              </div>
            ...
            gap: '12px'
              ...
              <button onClick={handleSimulateReturn} className="royal-btn" style={{
          ...
          gap: '12px'
            ...
            paddingBottom: '6px'
              ...
              <canvas ref={canvasRef} width={200} height={200} onClick={handleCanvasClick} style={{
            background: '#0d0d0d',
            ...
            fontSize: '0.7rem'
              ...
              }}></span> {t('auto_3270', 'Library Center')}
                </span>
                <span style={{
              display: 'flex',
              alignItems: 'center',
              ...
              color: 'rgba(0,191,255,0.9)'
                ...
                borderRadius: '3px',
                display: 'inline-block'
              }}></span> {t('auto_3271', 'Simulated Pin')}
                </span>
                <span style={{
              display: 'flex',
              alignItems: 'center',
              ...
              color: '#4caf50'
                ...
                borderRadius: '3px',
                display: 'inline-block'
              }}></span> {t('auto_3272', 'Verified Return')}
                </span>
                <span style={{
              display: 'flex',
              alignItems: 'center',
              ...
              color: '#f44336'
                ...
                borderRadius: '3px',
                display: 'inline-block'
              }}></span> {t('auto_3273', 'Unverified Return')}
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="ledger-splits-row">
        ...
        <div className="royal-card split-panel-card border-gold">
          ...
          {checkoutRequests.length === 0 ? <div className="ledger-empty-state">
              <CheckCircle size={32} className="text-success" />
              <p>{t('admin.noPendingCheckouts', 'No pending checkout requests from patrons.')}</p>
            </div> : <div className="requests-compact-list">
              {checkoutRequests.map(r => <div key={r.id} className="compact-request-row animate-fade-in">
                  ...
                  <div className="compact-row-actions">
                    <button onClick={() => handleApproveCheckout(r.id)} className="action-btn-circle btn-approve" disabled={actionLoading[r.id]} title={t('admin.approveCheckout', 'Approve Loan')}>
                      {actionLoading[r.id] === 'approve' ? <RefreshCw className="spin-icon" size={14} /> : <Check size={14} />}
                    </button>
                    <button onClick={() => handleRejectCheckout(r.id)} className="action-btn-circle btn-reject" disabled={actionLoading[r.id]} title={t('admin.rejectCheckout', 'Reject Loan')}>
        ...
        <div className="royal-card split-panel-card border-warning">
          ...
          gap: '10px'
            ...
            {preVerifiedReturns.length > 0 && <button onClick={handleBulkApproveReturns} className="royal-btn" style={{
            background: 'linear-gradient(135deg, #d4a574 0%, #b8860b 100%)',
            color: '#121212',
            border: 'none',
            padding: '6px 14px',
            ...
            e.currentTarget.style.boxShadow = '0 4px 15px rgba(212, 165, 116, 0.4)';
                ...
                <CheckCircle size={12} /> {t("str_5188", "Bulk Confirm (")}{preVerifiedReturns.length})
              </button>}
          </div>

          {returnRequests.length === 0 ? <div className="ledger-empty-state">
              <CheckCircle size={32} className="text-success" />
              <p>{t('admin.noPendingReturns', 'No pending return requests from patrons.')}</p>
            </div> : <div className="requests-compact-list">
              {returnRequests.map(r => <div key={r.id} className="compact-request-row animate-fade-in" style={{
            display: 'flex',
            justifyContent: 'space-between',
            alignItems: 'center',
            padding: '12px 16px',
            borderBottom: '1px solid rgba(255,255,255,0.05)'
                  ...
                  <div className="compact-row-meta" style={{
              display: 'flex',
              flexDirection: 'column',
              gap: '6px',
              flex: '1'
                ...
                flexWrap: 'wrap'
                      ...
                      <div className="request-time" style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: '4px',
                  fontSize: '0.8rem',
                  color: "var(--text-secondary)"
                      ...
                      <div className="compact-location-badge" style={{
                  display: 'flex',
                  gap: '6px',
                  alignItems: 'center'
                }}>
                        {r.locationVerified && <span className="location-badge verified" style={{
                    color: '#52c41a',
                    ...
                    gap: '4px'
                        ...
                        {r.qrVerified && <span className="qr-badge verified" style={{
                    color: '#52c41a',
                    border: '1px solid #52c41a',
                    background: 'rgba(82, 196, 26, 0.1)',
                    padding: '2px 8px',
                  ...
                  <div className="compact-row-actions">
                    <button onClick={() => handleApproveReturn(r.id)} className="action-btn-circle btn-approve-return" disabled={actionLoading[r.id]} title={t('admin.approveReturn', 'Approve Return')}>
      ...
      <section className="master-ledger-section royal-card">
        <div className="ledger-table-header-row">
          ...
          <div className="ledger-search-filters">
            ...
            <div className="search-bar-wrapper">
              ...
              <input type="text" placeholder={t('admin.searchCirculationPlaceholder', 'Search volume, ISBN, user, email...')} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="ledger-search-input" />
            </div>
            ...
            <div className="contact-toggle-wrapper">
              ...
              <button type="button" className={`luxury-toggle-btn ${showContactDetails ? 'active' : ''}`} onClick={() => setShowContactDetails(!showContactDetails)} title={t('admin.showPatronContacts', 'Show Patron Contacts')}>
            ...
            <div className="status-filter-wrapper">
              ...
              <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="ledger-filter-select">
        ...
        {loading ? <div className="ledger-table-loading">
          ...
          </div> : <div className="ledger-table-wrapper">
            <table className="ledger-table">
              ...
              <tbody>
                {getFilteredList().map(entry => <tr key={entry.id} className={`ledger-tr animate-fade-in ${isContactIncomplete(entry.memberId) ? 'contact-incomplete-warning' : ''}`}>
                    <td>{renderBookCell(entry.bookId, entry.copyNo)}</td>
                    <td>{renderMemberCell(entry.memberId, entry)}</td>
                    <td>
                      <div className="date-cell">
                        <span className="primary-date">
                          {entry.checkedOutAt ? new Date(entry.checkedOutAt).toLocaleDateString() : entry.requestedAt ? new Date(entry.requestedAt).toLocaleDateString() : '—'}
                        </span>
                        <span className="secondary-time">
                          {entry.checkedOutAt ? new Date(entry.checkedOutAt).toLocaleTimeString([], {
                      hour: '2-digit',
                      minute: '2-digit'
                    }) : entry.requestedAt ? new Date(entry.requestedAt).toLocaleTimeString([], {
                      hour: '2-digit',
                      minute: '2-digit'
                    }) : ''}
                        </span>
                    ...
                    <td>
                      <div className="date-cell">
                        <span className="primary-date">
                          {entry.returnedAt ? new Date(entry.returnedAt).toLocaleDateString() : entry.dueDate ? new Date(entry.dueDate).toLocaleDateString() : '—'}
                        </span>
                        {entry.status === 'CHECKED_OUT' && entry.dueDate && <span className={`due-badge ${new Date(entry.dueDate) < new Date() ? 'overdue' : 'pending'}`}>
                    ...
                    <td>
                      {entry.ntagUid ? <div className="ntag-badge">
                          <Smartphone size={12} className="text-accent" />
                          <span className="ntag-font font-mono">{entry.ntagUid}</span>
                        </div> : <span className="ntag-none">—</span>}
                    </td>
                    <td>
                      {entry.status === 'CHECKED_OUT' || entry.status === 'REQUESTED_RETURN' ? <button onClick={() => handleClearCheckout(entry.id)} className="clear-checkout-btn" disabled={actionLoading[entry.id] === 'clear'} title={t('admin.clearCheckout', 'Forcibly Clear Checkout')}>
                          {actionLoading[entry.id] === 'clear' ? <RefreshCw className="spin-icon" size={13} /> : <>
                              <X size={12} />
                              <span>{t('admin.clear', 'Clear')}</span>
                            </>}
                        </button> : <span className="action-none">—</span>}
                    </td>
                  </tr>)}
              </tbody>
            </table>
      ...
      <section className="physical-copy-registry-section royal-card" style={{
      marginTop: '40px'
    }}>
        <div style={{
        display: 'flex',
        ...
        borderRadius: '4px'
          ...
          margin: 0,
          fontSize: '0.9rem'
        }}>{t('auto_3280', 'Select a book volume above to inspect and manage its physical copies independently.')}</p>
          </div> : (() => {
        ...
        if (copiesList.length === 0) {
          return <div style={{
            ...
            borderRadius: '4px'
              ...
              margin: 0,
              fontSize: '0.9rem'
            }}>{t('auto_3281', 'No individual copies have been registered for this volume in the database yet.')}</p>
                </div>;
        }
        return <div style={{
          display: 'grid',
          ...
          gap: '20px'
            ...
            const activeTx = checkouts.find(c => c.bookId === selectedBookId && c.copyNo === copy.copyNo && ['CHECKED_OUT', 'REQUESTED_RETURN', 'REQUESTED_CHECKOUT'].includes(c.status));
              ...
              boxShadow: "0 4px 15px var(--card-shadow)"
                  ...
                  fontSize: '0.8rem'
                    ...
                    paddingBottom: '4px'
                            ...
                            <span style={{
                      color: 'var(--text-secondary)'
                    }}>{t("str_5191", "NTAG213 ID:")}</span>
                            <span className="font-mono" style={{
                      color: copy.ntagUid ? '#fff' : 'rgba(255,255,255,0.2)'
                    }}>{copy.ntagUid || '—'}</span>
                          </div>
                          <div style={{
                    display: 'flex',
                    ...
                    paddingBottom: '4px'
                            ...
                            <span style={{
                      color: 'var(--text-secondary)'
                    }}>{t("str_5192", "QR ID:")}</span>
                            <span className="font-mono" style={{
                      color: copy.qrId ? '#fff' : 'rgba(255,255,255,0.2)'
                    }}>{copy.qrId || '—'}</span>
                          </div>
                        </div>
                  ...
                  marginBottom: '16px'
                    ...
                    fontWeight: 'bold',
                    marginBottom: '6px'
                  }}>{t('auto_3282', 'Current Borrower')}</div>
                            <div style={{
                    fontSize: '0.85rem',
                    fontWeight: 'bold',
                    color: "var(--text-primary)"
                  ...
                  gap: '8px'
                            ...
                            {activeTx.status === 'REQUESTED_RETURN' && <button onClick={() => handleApproveReturn(activeTx.id)} className="royal-btn" style={{
                    width: '100%',
                    padding: '8px',
                    fontSize: '0.75rem',
                    background: 'linear-gradient(135deg, #4caf50 0%, #388e3c 100%)', color: '#ffffff',
                    border: 'none',
                    borderRadius: '4px',
                    fontWeight: 'bold',
                    cursor: 'pointer'
                            ...
                            {activeTx.status === 'REQUESTED_CHECKOUT' && <button onClick={() => handleApproveCheckout(activeTx.id)} className="royal-btn" style={{
                    width: '100%',
                    padding: '8px',
                    fontSize: '0.75rem',
                    background: 'linear-gradient(135deg, #d4a574 0%, #b8860b 100%)',
                    ...
                    cursor: 'pointer'
                            ...
                            <button onClick={() => handleClearCheckout(activeTx.id)} className="clear-checkout-btn" style={{
                    width: '100%',
                    padding: '8px',
                    fontSize: '0.72rem',
                    textAlign: 'center',
