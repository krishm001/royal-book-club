...
import { Sparkles, MessageSquare, Send, Calendar, Clock, Award, PenTool, Search, ChevronRight, BookText, HelpCircle, User, Image, MessageCircle, Plus, X, ArrowLeft, BookOpen, Trash2, Pencil } from 'lucide-react';
import { fetchDiscourses, fetchDiscourseById, publishDiscourse, commentOnChronicle, replyToDebate, updateDiscourse, deleteDiscourse, toggleDiscourseReaction, toggleCommentReaction, updateComment, deleteComment } from '../../services/discourseApi';
...
const DiscoursesPage = ({
  ...
  const handleUpdateCommentSubmit = async commentId => {
    ...
    try {
      ...
      if (res && res.success) {
        setChronicleComments(prev => prev.map(c => c.id === commentId ? {
          ...c,
  ...
  const toggleLocalReaction = (reactionsMap, reactionType, userId) => {
    const currentReactions = reactionsMap ? {
      ...reactionsMap
    } : {};
    ...
    if (targetType === 'chronicle' || targetType === 'debate') {
      ...
      setDiscourses(prev => prev.map(d => {
        if (d.id === id) {
          return {
            ...d,
            reactions: toggleLocalReaction(d.reactions, reactionType, userId)
          };
        }
        return d;
      }));
      ...
      setChronicleComments(prev => prev.map(c => {
        if (c.id === id) {
          return {
            ...c,
            reactions: toggleLocalReaction(c.reactions, reactionType, userId)
          };
        }
        return c;
      }));
      ...
      setDebateReplies(prev => prev.map(r => {
        if (r.id === id) {
          return {
            ...r,
            reactions: toggleLocalReaction(r.reactions, reactionType, userId)
          };
        }
        return r;
      }));
  ...
  const renderReactions = (node, targetType) => {
    ...
    const userId = user?.uid || user?.id;
    return <div className="socratic-reactions-wrapper">
        ...
        <div className="active-reactions-list">
          ...
          return <button key={emoji} type="button" onClick={() => handleToggleReaction(node.id, emoji, targetType)} className={`reaction-pill-badge ${hasReacted ? 'active' : ''}`} title={`${count} ${t('discourses.patronsReacted', 'patron(s) reacted')}`}>
        ...
        {user && <div className="reaction-picker-trigger-wrapper">
            ...
            const users = reactions[emoji] || [];
            const hasReacted = userId && users.includes(userId);
            return <button key={emoji} type="button" onClick={() => handleToggleReaction(node.id, emoji, targetType)} className={`picker-emoji-btn ${hasReacted ? 'active' : ''}`} title={emoji === "👍" ? t('discourses.concur', 'Concur') : emoji === "❤️" ? t('discourses.adore', 'Adore') : emoji === "💡" ? t('discourses.inspiriting', 'Inspiriting') : t('discourses.scholarly', 'Scholarly')}>
                    {emoji}
                  </button>;
          })}
            </div>
  ...
  const buildThreadedReplies = () => {
    ...
    const renderReplyCard = (reply, depth, rootReplyId) => {
      ...
      return <div key={reply.id} className={`debate-reply-node ${isMe ? 'reply-mine' : 'reply-other'} depth-${depth}`}>
          <div className={`debate-reply-card royal-card glassmorphic ${isMe ? 'bubble-mine' : 'bubble-other'} ${longPressedReplyId === reply.id ? 'actions-active' : ''}`} onTouchStart={() => handleTouchStart(reply.id)} onTouchEnd={handleTouchEnd} onTouchMove={handleTouchMove}>
            
            {/* Citation Quote Card for flattened sub-replies */}
            {isFlattened && parentReply && <div className="socratic-citation-card">
            ...
            <div className="reply-header">
              <div className="reply-author-info">
                {reply.authorPhotoUrl ? <img src={reply.authorPhotoUrl} alt={reply.authorName} className="reply-avatar" /> : <div className="reply-avatar-fallback"><User size={12} /></div>}
              ...
              <span className="reply-date">
                ...
                hour: '2-digit',
                minute: '2-digit'
              }) : t('discourses.justNow', 'Just now')}
              </span>
            </div>

            {editingReplyId === reply.id ? <form onSubmit={e => handleSaveEditReply(e, reply)} className="reply-edit-form animate-fade-in" style={{
            marginTop: '8px',
            padding: '4px'
          }}>
                <textarea className="royal-input reply-edit-textarea" value={editingReplyText} onChange={e => setEditingReplyText(e.target.value)} required rows={2} />
              ...
              </form> : <p className="reply-text-content">{reply.content}</p>}
              ...
              {user && <div className="reply-actions">
                ...
                setReplyText('');
                  ...
                  {(user.uid === reply.authorId || user.id === reply.authorId || user.role === 'ADMIN') && <>
                      <button onClick={() => handleStartEditReply(reply)} className="reply-edit-btn">
                        <PenTool size={12} /> <span className="btn-label-text">{t('discourses.edit')}</span>
                      </button>
                      <button onClick={() => handleDeleteReply(reply.id)} className="reply-delete-btn">
            ...
            {replyInputId === reply.id && <form onSubmit={e => handlePostReply(e, reply.id)} className="reply-submit-form animate-fade-in">
                <input type="text" placeholder={`${t('discourses.replyTo', 'Reply to')} ${reply.authorName}...`} className="royal-input reply-input" value={replyText} onChange={e => setReplyText(e.target.value)} required autoFocus />
    ...
    return <div className="debate-discussion-tree">
        {rootReplies.length > 0 ? rootReplies.map(rootReply => {
        const descendants = getDescendants(rootReply.id);
        return <div key={rootReply.id} className="debate-root-reply-group">
                ...
                {descendants.length > 0 && <div className="nested-replies-container">
  ...
  return <div className="discourses-page-container animate-fade-in">
      
      {/* Immersive Chronicle Detailed Modal */}
      {selectedChronicle && <div className="focal-detail-overlay animate-fade-in" onClick={handleCloseChronicle}>
          <div className="focal-detail-drawer" onClick={e => e.stopPropagation()}>
            ...
            {loadingDetail ? <div className="focal-loading">
                <div className="loader-mini"></div>
                <p>{t('discourses.retrievingSacredChronicles', 'Retrieving sacred chronicles...')}</p>
              </div> : chronicleDetail ? <div className="focal-content-scroll">
                ...
                <div className="focal-body">
                  ...
                  {user && (user.uid === chronicleDetail.authorId || user.id === chronicleDetail.authorId || user.role === 'ADMIN') && <div style={{
              display: 'flex',
              gap: '8px',
              marginTop: '12px',
              marginBottom: '12px',
              flexWrap: 'wrap'
                      ...
                      <button onClick={() => handleStartEdit(chronicleDetail)} className="royal-btn edit-discourse-btn" style={{
                display: 'flex',
                alignItems: 'center',
                gap: '6px',
                padding: '6px 12px',
                fontSize: '0.85rem'
                      ...
                      <button onClick={() => handleDelete(chronicleDetail.id)} className="royal-btn delete-discourse-btn" style={{
                display: 'flex',
                alignItems: 'center',
                gap: '6px',
                padding: '6px 12px',
                ...
                color: "var(--text-primary)"
                  ...
                  <div className="focal-content-html" dangerouslySetInnerHTML={{
              __html: chronicleDetail.content
            }} />

                  {chronicleDetail.tags && chronicleDetail.tags.length > 0 && <div className="focal-tags-container">
                      {chronicleDetail.tags.map(t => <span key={t} className="royal-tag">#{t}</span>)}
                    </div>}
                  ...
                  return <div key={c.id} className="comment-card royal-card">
                            <div className="comment-card-header">
                              <div className="comment-user">
                                {c.authorPhotoUrl ? <img src={c.authorPhotoUrl} alt={c.authorName} className="comment-avatar" /> : <div className="comment-avatar-fallback"><User size={12} /></div>}
                                <span className="comment-user-name gold-gradient-text">{c.authorName}</span>
                              </div>
                              <div className="comment-header-right" style={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: '0.75rem'
                      }}>
                            ...
                            day: 'numeric'
                                ...
                                {!isEditing && (isAuthor || isAdmin) && <div className="comment-actions" style={{
                          display: 'flex',
                          alignItems: 'center',
                          gap: '0.5rem'
                        }}>
                                    {isAuthor && <button onClick={() => handleStartEditComment(c.id, c.content)} className="comment-action-btn edit-btn" title={t('discourses.editInsight')} style={{
                            background: 'none',
                            border: 'none',
                            ...
                            alignItems: 'center'
                                    ...
                                    <button onClick={() => handleDeleteCommentClick(c.id)} className="comment-action-btn delete-btn" title={t('discourses.purgeInsight')} style={{
                            background: 'none',
                            border: 'none',
                            color: '#ff4d4d',
                            cursor: 'pointer',
                            ...
                            {isEditing ? <div className="comment-edit-form" style={{
                      marginTop: '0.5rem'
                    }}>
                                <textarea className="royal-input comment-textarea edit-mode" value={editingCommentText} onChange={e => setEditingCommentText(e.target.value)} rows={2} style={{
                        width: '100%',
                        marginBottom: '0.5rem'
                      }} />
                                <div className="comment-edit-actions" style={{
                        display: 'flex',
                        gap: '0.5rem'
                      }}>
                                  <button onClick={() => handleUpdateCommentSubmit(c.id)} className="royal-btn small-btn save-btn" style={{
                          padding: '0.25rem 0.75rem',
                          fontSize: '0.8rem'
                        }}>
                                    {t('common.update', 'Update')}
                                  </button>
                                  <button onClick={handleCancelEditComment} className="royal-btn small-btn cancel-btn" style={{
                          padding: '0.25rem 0.75rem',
                          fontSize: '0.8rem',
                          ...
                          color: '#ccc'
                              ...
                              </div> : <p className="comment-text-content">"{c.content}"</p>}
      ...
      <header className="discourses-header">
        ...
        <div className="royal-tabs-container">
          <button className={`royal-tab-btn ${activeTab === 'CHRONICLE' ? 'active' : ''}`} onClick={() => {
          setActiveTab('CHRONICLE');
          setIsCreating(false);
        }}>
            <BookText size={16} /> {t('discourses.intellectualChronicles', 'Intellectual Chronicles')}
          </button>
          <button className={`royal-tab-btn ${activeTab === 'DEBATE' ? 'active' : ''}`} onClick={() => {
          setActiveTab('DEBATE');
          setIsCreating(false);
        }}>
      ...
      {user && <section className="composition-orchestrator">
        ...
        setFormType(activeTab);
            ...
            </button> : <div className="royal-card composition-card animate-fade-in">
            ...
            resetForm();
              ...
              <form onSubmit={handlePublish} className="composition-form">
                ...
                {formType === 'CHRONICLE' && <div className="form-row-grid">
                    ...
                    <div className="form-group">
                      ...
                      <div className="flyer-upload-zone">
                        <input type="file" id="chronicle-cover-file" accept="image/*" onChange={handleFileChange} style={{
                  display: 'none'
                }} />
                        <label htmlFor="chronicle-cover-file" className="flyer-upload-trigger">
                          <Image size={16} /> {t('discourses.uploadBanner', 'Upload Banner')}
                        </label>
                        {coverPreview && <div className="flyer-upload-preview">
                ...
                <div className="form-group">
                  ...
                  {formType === 'CHRONICLE' ? <RichTextEditor value={content} onChange={setContent} placeholder={t('discourses.dissertationPlaceholder', 'Draft your exquisite dissertation...')} /> : <textarea className="royal-input debate-textarea" placeholder={t('discourses.debateBoundsPlaceholder', 'Describe the philosophical bounds or query of your debate...')} value={content} onChange={e => setContent(e.target.value)} required rows={5} />}
                </div>

                {formType === 'CHRONICLE' && <div className="form-group">
                    <label className="royal-label">{t('discourses.customTagsLabel', 'Custom Tags (Press Enter after each)')}</label>
                    <input type="text" className="royal-input" placeholder={t('discourses.typeTagPlaceholder', 'Type tag and press Enter...')} value={tagInput} onChange={e => setTagInput(e.target.value)} onKeyDown={handleAddTag} />
                    {tagsList.length > 0 && <div className="tags-display-row">
                        {tagsList.map(t => <span key={t} className="royal-tag">
      ...
      <section className="search-filter-section royal-card glassmorphic">
        <div className="search-wrapper">
          ...
          <input type="text" placeholder={activeTab === 'CHRONICLE' ? t('discourses.searchChroniclesPlaceholder', 'Search chronicles by title, tags, or author...') : t('discourses.searchDebatesPlaceholder', 'Search debates by title, tags, or author...')} className="royal-input search-input" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
        </div>
      </section>
      ...
      <main className="discourses-main-feed">
        {loading ? <div className="loading-boundary">
            ...
            {activeTab === 'CHRONICLE' && filteredDiscourses.map(disc => <div key={disc.id} className="chronicle-card royal-card glassmorphic animate-fade-in">
                ...
                <div className="chron-content">
                  <div className="chron-meta">
                    <span className="chron-date"><Calendar size={12} /> {new Date(disc.createdAt).toLocaleDateString('en-US', {
                  month: 'short',
                  day: 'numeric',
                  year: 'numeric'
                })}</span>
                  ...
                  <div className="chron-excerpt" dangerouslySetInnerHTML={{
              __html: disc.content.substring(0, 160) + '...'
            }} />
          ...
          return <div key={disc.id} className={`debate-thread-card royal-card glassmorphic animate-fade-in ${isExpanded ? 'expanded' : ''}`}>
                    ...
                    <div className="debate-title-block">
                      ...
                      <div className="title-text">
                        ...
                        <div className="debate-meta">
                          ...
                          <span>•</span>
                          <span>{new Date(disc.createdAt).toLocaleDateString('en-US', {
                        month: 'short',
                        day: 'numeric',
                        year: 'numeric'
                      })}</span>
                ...
                handleOpenDebate(disc);
                  ...
                  {isExpanded && <div className="debate-expanded-body animate-fade-in">
                      ...
                      {user && (user.uid === disc.authorId || user.id === disc.authorId || user.role === 'ADMIN') && <div style={{
                display: 'flex',
                gap: '8px',
                marginBottom: '16px',
                flexWrap: 'wrap'
              }}>
                          <button onClick={() => handleStartEdit(disc)} className="royal-btn edit-discourse-btn" style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: '4px',
                  padding: '4px 8px',
                  fontSize: '0.8rem'
                          ...
                          <button onClick={() => handleDelete(disc.id)} className="royal-btn delete-discourse-btn" style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: '4px',
                  padding: '4px 8px',
          ...
          </div> : <div className="royal-card no-discourses-fallback">
