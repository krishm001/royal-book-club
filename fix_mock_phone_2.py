...
# I will replace from `<div className="mock-phone-body-state">` up to `      {/* 6. Zoomed QR Scanner Overlay */}`
pattern = r'<div className="mock-phone-body-state">[\s\S]*?(?=      \{/\* 6\. Zoomed QR)'

replacement = '''<div className="mock-phone-body-state" style={{ flex: 1, position: 'relative' }}>
        ...
        {type === 'barcode' && (
          <div className="mock-scan-body" style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <div style={{ width: '38px', height: '18px', background: tPalette.viewfinderBg, border: `1px solid ${tPalette.accent}`, borderRadius: '2px', margin: '4px 0', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ fontSize: '4.5px', fontWeight: 'bold', color: tPalette.textPrimary, textAlign: 'center', margin: '2px 0' }}>
              {isReturn ? t('catalog.animScanReturnQr', 'Scan return QR code') : t('catalog.animScanQr', 'Scan library QR code')}
            </div>
          </div>
        )}
        ...
        {type === 'nfc' && (
          <div className="mock-scan-body" style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <div style={{ fontSize: '4.5px', fontWeight: 'bold', color: tPalette.textPrimary, textAlign: 'center', margin: '6px 0 2px' }}>
              {t('catalog.animNfcTap', 'Tap phone on book cover')}
            </div>
          </div>
        )}
        ...
        <div className="mock-success-body">
          {isConfirmation ? (
            <div style={{ background: tPalette.surface, border: `1px solid ${tPalette.accent}`, padding: '4px', margin: '4px', borderRadius: '3px', zIndex: 10 }}>
              <div style={{ fontSize: '5px', fontWeight: 'bold', color: tPalette.accent, textAlign: 'center', marginBottom: '2px' }}>{t('catalog.animCheckoutComplete', 'Checkout complete!')}</div>
              ...
              <div style={{ fontSize: '3px', color: tPalette.textSecondary, marginBottom: '2px' }}>{t('catalog.p2dDesc', 'A physical possession barcode or NFC match has been registered.')}</div>
              <div style={{ background: tPalette.accent, color: '#fff', fontSize: '3.5px', fontWeight: 'bold', textAlign: 'center', padding: '2px', borderRadius: '1.5px' }}>{t('common.confirm', 'CONFIRM')}</div>
            </div>
          ) : (
            <div style={{ background: tPalette.surface, border: `1px solid ${tPalette.accent}`, padding: '4px', margin: '4px', borderRadius: '3px', zIndex: 10 }}>
              ...
              <div style={{ fontSize: '4.5px', fontWeight: 'bold', color: tPalette.textPrimary, textAlign: 'center', marginBottom: '1px' }}>{t('catalog.royalVerification', 'Royal Verification Confirmed')}</div>
              <div style={{ fontSize: '3px', color: tPalette.textSecondary, textAlign: 'center', marginBottom: '3px' }}>{t('catalog.howExperience', 'How was your experience?')}</div>
              ...
              <div style={{ background: tPalette.accent, color: '#fff', fontSize: '3.5px', fontWeight: 'bold', textAlign: 'center', padding: '2px', borderRadius: '1.5px' }}>{isReturn ? t('catalog.writeReview', 'WRITE REVIEW') : t('catalog.viewGatepass', 'VIEW GATEPASS')}</div>
            </div>
          )}
        </div>
      </div>\n\n'''
