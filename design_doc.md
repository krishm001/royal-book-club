...
9. **Dynamic Action Routing**: The intermediate confirmation popup must be bypassed for Book Card and Book Detail scanners. The Top Scanner must dynamically query the database *first* before determining which confirmation action to show, and must bypass the confirmation on pre-validation errors. All scanners route errors into a full-screen modal. **Crucially, the NFC tap flow and the QR scan flow must behave identically and interchangeably across all scanner entry points.**
